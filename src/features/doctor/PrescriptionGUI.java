...
public class PrescriptionGUI extends javax.swing.JFrame {
	...
	public void refreshTableData(){
		try {
			...
			for(Appointment appointment:prescriptionTable.getAppointments()){
				if(appointment.getApptStatus().getAppointmentStatusDescription().equalsIgnoreCase("Present")){
	...
	public void updateAppointmentStatus(String appointmentId){
		...
		for(int i=0;i< appointments.size();i++){
			if(appointments.get(i).getAppointmentId().equals(appointmentId)&&
					appointments.get(i).getApptStatus().getAppointmentStatusDescription().equalsIgnoreCase("Confirmed")){
				...
				if(!medicineStatus.equalsIgnoreCase("Incomplete")&&
					...
					appointmentService.saveDataToFile();
	...
	public boolean hasOperationHours(){
		try {
			Map<String, List<OperationSchedule>> hours = OperationScheduleService.loadOperationHours(userId);
