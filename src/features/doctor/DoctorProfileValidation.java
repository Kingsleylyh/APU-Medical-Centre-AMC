...
public class DoctorProfileValidation {
	...
	private Doctor doctorProfile = new Doctor();
	...
	public void loadData(){
		doctors = UserService.getDoctorList();
	...
	public String getDoctorDob(){
		return doctorProfile.getDob().format(formatter);
	}
	...
	this.doctors = UserService.getDoctorList();
    }

    public Map<String,List<OperationSchedule>> loadOperationHours(){
        try{
            return OperationScheduleService.loadOperationHours(userId);
        } catch (IOException e) {
    ...
    public boolean saveOperationHours(Map<String,List<TimeBlock>> operationHours){
        try{
            ...
            return OperationScheduleService.updateOperationHours(userId, getDoctorUserId(), processedHours);
        } catch (Exception e) {
    ...
    public int validateUsername(String username){
        ...
        for(Doctor doctor : doctors){
            if(username.equals(doctor.getUsername()) && !doctor.getUserId().equals(doctorProfile.getUserId())){
    ...
    public boolean updateProfile(String userId,String name,String username, String password, String email) {
        ...
        try {
            ...
            for (int i = 0; i < doctors.size(); i++) {
                if (doctors.get(i).getUserId().equals(doctorProfile.getUserId())) {
                    doctors.set(i, doctorProfile);
                    break;
                }
            }
            DoctorFileManager.saveUsers(doctors);
            return true;
        } catch (Exception e) {
